...
\begin{figure}[H]
  \centering
  % Uncomment the line below if 'overall_system.png' is available
...
As shown in Figure \ref{fig:overall_arch}, the CPU requests flow sequentially:
\begin{enumerate}
  \item \textbf{CPU $\rightarrow$ MMU:} The CPU issues \texttt{virt\_addr} along with \texttt{read\_mem} or \texttt{write\_mem} control signals.
  \item \textbf{MMU $\rightarrow$ Cache Controller:} Upon a successful TLB hit, the MMU forwards the translated \texttt{phy\_addr} to the Cache Controller.
  \item \textbf{Cache Controller $\rightarrow$ Memory:} If a cache miss occurs, the controller communicates with Main Memory via the \texttt{addr} and \texttt{data} buses to fetch the block.
\end{enumerate}

\subsection{Memory Management Unit (MMU) Implementation}
We implemented a fully associative TLB model within the MMU. Figure \ref{fig:mmu_block} illustrates the MMU's central role in translation.

\begin{figure}[H]
  \centering
  % Uncomment the line below if 'mmu_block.png' is available
  \includegraphics[width=0.8\textwidth]{mmu_block.png}
  \caption{Memory Management Unit (MMU) Interface.}
  \label{fig:mmu_block}
\end{figure}

\begin{figure}[H]
  \centering
  % Uncomment the line below if 'tlb_block.png' is available
  \includegraphics[width=0.8\textwidth]{tlb_block.png}
  \caption{Translation Lookaside Buffer (TLB) Block Diagram.}
  \label{fig:tlb_block}
\end{figure}

The MMU integrates the TLB (Figure \ref{fig:tlb_block}), which performs the critical lookup.
\begin{itemize}
  \item \textbf{Lookup Logic:} The TLB compares the incoming \texttt{virt\_page\_number} against all valid tags simultaneously in a single clock cycle.
  \item \textbf{Refill Logic:} Upon a miss, the system signals a miss condition via \texttt{TLB\_hit/miss}, which triggers a simulated Page Table Walk in the testbench.
\end{itemize}

\subsection{Cache Controller Implementation}
The Cache Controller is the most complex module, managing data flow and consistency. As depicted in Figure \ref{fig:cc_block}, it sits between the MMU, the Cache Memory (SRAM), and the CPU.

\begin{figure}[H]
  \centering
  % Uncomment the line below if 'cache_controller.png' is available
...
\begin{itemize}
  \item \textbf{To CPU:} Provides \texttt{hit/miss} status and \texttt{ready/stall} signals to freeze the pipeline during misses.
  \item \textbf{To Cache Mem:} As shown in Figure \ref{fig:cache_mem}, this separate module stores the actual data. The controller sends indices and write enables to this block.
  \item \textbf{To Main Memory:} As shown in Figure \ref{fig:main_mem}, this interface handles block refills (512 bits) and write-through operations.
\end{itemize}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    \centering
    % Uncomment the line below if 'cache_memory.png' is available
    \includegraphics[width=\textwidth]{cache_memory.png}
    \caption{Cache Memory (Data Store)}
    \label{fig:cache_mem}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \centering
    % Uncomment the line below if 'main_memory.png' is available
    \includegraphics[width=\textwidth]{main_memory.png}
    \caption{Main Memory Interface}
    \label{fig:main_mem}
  \end{subfigure}
  \caption{Memory Storage Modules.}
\end{figure}
